    ...
    for (var metric in summary[namespace]) {
      ...
      switch(summary[namespace][metric].type) {
        ...
        case 'timer':
          ...
          break;
        case 'gauge':
          var gaugeArray = summary[namespace][metric].points;
          var arrayLength = gaugeArray.length;
          for (var i = 0; i < gaugeArray.length; i++) {
            var gaugeFields = {};
            gaugeFields['count'] = { type: 'integer', value: gaugeArray[i].value }
            var tags = objectAssign(this.tagger(key), this.tags);
            this._influx.addPoint(key, tags, gaugeArray[i].timestamp, gaugeFields);
          }
          continue;
        default:
          continue;
